...
export class HashGenerator {
  ...
  static async generateHashFromFile(algorithm: HashAlgorithm, fileContent: ArrayBuffer): Promise<string> {
    ...
    try {
      ...
      throw new Error(`Failed to generate ${algorithm} hash from file: ${error}`);
    }
  }

  private static async webCryptoHashFromBuffer(algorithm: string, data: ArrayBuffer): Promise<string> {
    const hashBuffer = await crypto.subtle.digest(algorithm, data);
    const hashArray = Array.from(new Uint8Array(hashBuffer));
    return hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
  }

  static getAlgorithmInfo(algorithm: HashAlgorithm): { name: string; description: string; outputLength: string } {
    const info = {
      md5: {
        name: 'MD5',
        description: 'Message Digest Algorithm 5 - Fast but not cryptographically secure',
      ...
      sha512: {
        name: 'SHA-512',
        description: 'Secure Hash Algorithm 512-bit - More secure, larger output',
